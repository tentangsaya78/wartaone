  ...
  <div class="p-4">
    <u-form @submit.prevent="handleUpdate" class="space-y-4 max-w-md">
      ...
      <u-button type="submit"  trailing-icon="bi-send" :loading="isSubmitting"
        :disabled="isSubmitting || !formIuran.title" class="max-w-max">
...
watch(
  ...
  (val) => {
    if (val) {
      formIuran.title = val.title || ''
      formIuran.content = val.content || ''
...
const uploadMedia = async (file: File) => {
  ...
  formData.append('alt_text', `Bukti pembayaran - ${formIuran.title}`)
...
const handleUpdate = async () => {
  ...
  try {
    ...
    if (formIuran.author) {
      postData.author = formIuran.author
    }
    ...
    await $fetch(`${wpUrl}/wp/v2/iuran/${route.params.id}`, {
      method: 'POST',
      headers: {
        Authorization: `Bearer ${token.value}`,
