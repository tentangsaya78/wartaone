<template>
  <div class="p-4 space-y-4">
    <!-- create button -->
    <div>
     <UButton to="/dashboard/admin/iuran/create" color="primary" class="mb-4" trailing-icon="i-heroicons-plus">
        Konfirmasi
      </UButton>
    </div>
        ...
        <tbody class="bg-white divide-y divide-gray-200">
          <tr v-for="(row, index) in paginatedData" :key="row.id" class="hover:bg-gray-50 transition-colors">
            ...
            <td class="px-6 py-4 whitespace-nowrap">
              <div class="flex items-center gap-2">
                <UButton
                  :to="`/dashboard/admin/iuran/detail/${row.slug}`"
                  color="primary"
                  variant="ghost"
                  size="xs"
                ...
                />
                <UButton
                  :to="`/dashboard/admin/iuran/edit/${row.id}`"
                  color="gray"
                  variant="ghost"
                  size="xs"
   ...
   <!-- Pagination -->
    <UPagination 
      v-if="iuran?.pagination?.total > itemsPerPage" 
...
definePageMeta({
  layout: 'admin',
  title: 'Data Iuran',
  description: 'Kelola dan lihat data iuran bulanan admin',
})
...
const { data: iuran, pending, error, refresh } = await request('api/v1/posts/iuran', {
  headers: {
...
const totalPages = computed(() => iuran?.value?.pagination?.total_pages || 1)
const totalItems = computed(() => iuran?.value?.pagination?.total || filteredData.value.length)
