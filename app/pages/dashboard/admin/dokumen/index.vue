<template>
  <div class="p-4 space-y-4">
    <!-- create button -->
    <div>
     <UButton to="/dashboard/warga/dokumen/create" color="primary" class="mb-4" trailing-icon="i-heroicons-plus">
        Tambah Dokumen
      </UButton>
    </div>
        ...
        <tbody class="bg-white divide-y divide-gray-200">
          <tr v-for="(row, index) in paginatedData" :key="row.id" class="hover:bg-gray-50 transition-colors">
            ...
            <td class="px-6 py-4 whitespace-nowrap">
              <div class="flex items-center gap-2">
                <UButton
                  :to="`/dashboard/warga/dokumen/detail/${row.slug}`"
                  color="primary"
                  variant="ghost"
                  size="xs"
                ...
                />
                <UButton
                  :to="`/dashboard/warga/dokumen/edit/${row.id}`"
                  color="gray"
                  variant="ghost"
                  size="xs"
   ...
   <!-- Pagination -->
    <UPagination 
      v-if="dukumen?.pagination?.total > itemsPerPage" 
    ...
    <!-- Empty State -->
    <div v-if="filteredData.length === 0 && !pending" class="text-center py-12">
...
definePageMeta({
  layout: 'admin',
  title: 'Dokumen',
  description: 'Kelola dan lihat data dokumen',
})
...
const { data: dukumen, pending, error, refresh } = await request('api/v1/posts/dokumen', {
  headers: {
...
const dataUtable = computed(() => {
  if (!dukumen.value?.data) return []
  
  const list = Array.isArray(dukumen.value.data) ? dukumen.value.data : []
...
const filteredData = computed(() => {
  ...
  if (searchTitle.value.trim()) {
    const search = searchTitle.value.toLowerCase()
...
const totalPages = computed(() => dukumen?.value?.pagination?.total_pages || 1)
const totalItems = computed(() => dukumen?.value?.pagination?.total || filteredData.value.length)
